...
<form action="{{ route('admin.emploi.search') }}" method="GET"
    ...
    <a href="{{ route('admin.emploi.index') }}" class="btn btn-secondary">
...
while ($time <= $endTime) {
    ...
    @php $skip = []; @endphp
    <h4 class="mt-4 mb-2" style="font-weight: 400; font-size: 1.3rem;">{{ $groupe->libelle }}</h4>
        ...
        <table class="emploi-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    if (
                    $e->moduleGroupeEnseignant->groupe->id == $groupe->id &&
                    $eventDayNum == $numJour &&
                    ...
                    for ($i = 1; $i < $span; $i++) {
                        ...
                        <td class="seance-td {{ $event->type === 'examen' ? 'examen-td' : '' }}" rowspan="{{ $span }}">
                            <div class="seance-cell">
                                ...
                                <small>
                                    {{ $event->moduleGroupeEnseignant->enseignant->user->nom }}
                                    {{ $event->moduleGroupeEnseignant->enseignant->user->prenom }}
                                </small>
