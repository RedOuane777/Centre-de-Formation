...
<div class="d-flex flex-wrap gap-2 mb-3">
    <a href="{{ route('admin.modules.create') }}" class="btn btn-primary">
        <i class="bi bi-plus-circle me-1"></i> Ajouter Module
    </a>

    <form action="{{ route('admin.modules.destroyAll') }}" method="POST"
        onsubmit="return confirm('Voulez-vous vraiment supprimer touts les modules ?')">
        @csrf
        @method('DELETE')
...
<form action="{{ route('admin.modules.index') }}" method="GET"
    ...
    <input type="text" name="search" class="form-control"
        placeholder="Rechercher par module ou filière"
    ...
    <div class="table-responsive shadow-sm rounded">
        <table class="table table-sm table-compact custom-table mb-0">
            ...
            <tbody>
                ...
                <tr>
                    <td title="{{ $module->titre }}">{{ $module->titre }}</td>
                    <td title="{{ $module->filiere->libelle ?? '' }}">{{ $module->filiere->libelle ?? '' }}</td>
                    <td>{{ $module->heures }}</td>
                    <td>
                        <a href="{{ route('admin.modules.edit', $module->id) }}" class="btn btn-warning btn-sm mb-1">
                            <i class="bi bi-pencil-square me-1"></i> Modifier
                        </a>
                        <form action="{{ route('admin.modules.destroy', $module->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-danger btn-sm mb-1" onclick="return confirm('Voulez-vous vraiment supprimer cet module ?')">
